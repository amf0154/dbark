...
try:
    ...
    logger.warning("Google API библиотеки не установлены. Google Drive функции недоступны.")
    GOOGLE_LIBS_AVAILABLE = False

class GoogleDriveBackup:
    def __init__(self):
        if not GOOGLE_LIBS_AVAILABLE:
            raise ImportError("Google API библиотеки не установлены")
    ...
    def upload_file(self, file_path, folder_id=None):
        ...
        try:
            ...
            # Создание папки для бэкапов MySQL если не указана конкретная папка
            if not target_folder_id:
                target_folder_id = self.create_folder_if_not_exists('MySQL-Backups')
        ...
        try:
            ...
            if not target_folder_id:
                ...
                results = self.service.files().list(
                    q="name='MySQL-Backups' and mimeType='application/vnd.google-apps.folder'"
                ).execute()
                items = results.get('files', [])
                if not items:
    ...
    def _search_backups_recursive(self, folder_id, all_files):
        ...
        try:
            ...
            for file in files:
                ...
                elif (file['name'].endswith('.sql.gz') or file['name'].endswith('.sql')) and \
                     ('backup_' in file['name'] or 'm2_' in file['name'] or 'mysql_' in file['name']):
                    # Это файл бэкапа MySQL с любым префиксом
...
def main():
    ...
    if not GOOGLE_LIBS_AVAILABLE:
        ...
        print("Для использования Google Drive функций установите:")
    ...
    try:
        ...
        if command == 'upload' and len(sys.argv) > 2:
            ...
            if success:
                ...
                print(f"Ошибка скачивания файла: {filename}")
